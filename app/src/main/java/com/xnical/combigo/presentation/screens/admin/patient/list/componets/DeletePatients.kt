...
@Composable
fun DeletePatients( vm: AdminPatientListViewModel = hiltViewModel()) {
    when(val response = vm.patientDeleteResponse) {
        Resource.Loading -> {
            ProgressBar()
        }
        is Resource.Success -> {
            Toast.makeText(LocalContext.current, "El paciente se elimino correctamente", Toast.LENGTH_LONG).show()
        }
        is Resource.Failure -> {
